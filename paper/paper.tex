...
\end{enumerate}  
Conjunctions of formal declarations of the above explicit and implicit conditions form the semantics of the transition label transfer-elect.   The predicate TransferAuxSpec defines  the semantics of this transition.
 \begin{small}
\begin{holthmenv}
...
\end{holthmenv}
\end{small}
We augment the formal counterparts of the $\bullet_{i}$ conditions  to the clauses given in Section~\ref{sec:MachineSem} to obtain the specification TransferActSpec for transfer-elect of ACT STV.  
...
The lines denoted by the star symbol~($\star$) in Definition of TransferActSpec formalise the Clause~$\bullet_{7}$ above. Also note that we place the last parcel of the pile of the candidate $c$ whose votes are transferred first into the list of uncounted ballots so that in the subsequent transition count deals with distributing the votes and ``recalculating'' candidates' tallies and piles. Finally, because of efficiency purposes our system is designed to update the fractional transfer value of the surplus in formalisation of the elect transition instead of the transfer-elected transition. 
...
\;\;\;\;\HOLSymConst{\HOLTokenNeg{}}\HOLConst{null}\;(\HOLConst{getCandPile}\;\HOLBoundVar{hbl}\;\HOLFreeVar{p})\;\HOLSymConst{\HOLTokenConj{}}\;\HOLConst{ALL_EMPTY}\;\HOLFreeVar{l}\;\HOLFreeVar{p\sp{\prime}}\;\HOLSymConst{\HOLTokenConj{}}\;\HOLFreeVar{h\sp{\prime}}\;\HOLSymConst{=}\;\HOLFreeVar{l}\\
